...
write.table(gene.info[gene.info$genename %in% markers.imm,],paste0(my.path,"/fig3/glist_fig3panelD2F_moduleIMM.txt"),
            sep = "\t",quote = F)
write.table(gene.info[rownames(gene.info) %in% markers.red$nodeName,],paste0(my.path,"/fig3/glist_fig3panelD2F_moduleRED.txt"),
            sep = "\t",quote = F)
...
#for fig.4G
my.path="/scicore/home/tschoppp/GROUP/lab/skeletalconvergence/fromMenghan/figure_drafts"
gene.info[gene.info$genename %in% c("SOX9","SOX5","FOXP3"),]

subset.rna@active.assay="RNA"
meta.rna=my.mfiles[[origin]]
subset.rna=AddMetaData(subset.rna,meta.rna[rownames(subset.rna@meta.data),c("annotation_broad","annotation_fine")])
subset.rna@meta.data$anno.tmp=subset.rna@meta.data$annotation_fine
subset.rna@meta.data[subset.rna@meta.data$anno.tmp!="chondrocytes","anno.tmp"]="otherMesenchyme"
Idents(subset.rna)=factor(subset.rna$anno.tmp,levels = c("otherMesenchyme","chondrocytes"))
p=VlnPlot(subset.rna,features = c("ENSGALG00000004386","ENSGALG00000032768","ENSGALG00000007769"),pt.size = 0.1)
pdf(paste0(my.path,"/fig4/vlnplot_geneExp_",origin,".pdf"), width=10, height=6)
p
dev.off()
...
assay.used="RNA"
my.average=AverageExpression(subset.rna,assays = assay.used)[[assay.used]]
...
dynamics.rna=my.average[c("SOX9","SOX5","FOXP3"),]
dynamics.rna=t(scale(t(dynamics.rna)))

quantile(dynamics.rna);break3=seq(-1.5, 1.5, by=0.025)
...
save_pheatmap_pdf(pt1.rna, paste0(my.path,"/fig4/fig4G_geneExpHeatmap_",origin,".pdf"), width=6, height=3)
```
