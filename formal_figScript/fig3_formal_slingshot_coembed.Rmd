...
for(i in 1:length(samples)){
  ...
  # get imputed expression for scATAC
  refdata <- GetAssayData(my.rna, assay = "integrated", slot = "data")[hvgs,]
  predicted.expression1 <- TransferData(
    anchorset = transfer.anchors,
    refdata = refdata,
    weight.reduction = my.atac[[embed.transfer]],
    dims = 2:30
  )
  my.atac[["imputed.integrated"]]=CreateAssayObject(data=predicted.expression1@data[,rownames(my.atac@meta.data)])
  
  refdata2 <- GetAssayData(my.rna, assay = "RNA", slot = "data")[hvgs,]
  predicted.expression2 <- TransferData(
    anchorset = transfer.anchors,
    refdata = refdata2,
    weight.reduction = my.atac[[embed.transfer]],
    dims = 2:30
  )
  my.atac[["imputed.rna"]]=CreateAssayObject(data=predicted.expression2@data[,rownames(my.atac@meta.data)]);gc()
  
  
  #################
  ## option1: MapQuery, project on scRNA (only UMAP)
  ...
  DefaultAssay(my.atac) <- "imputed.integrated"
  DefaultAssay(my.rna) <- "integrated"
  my.anchors <- FindTransferAnchors(reference = my.rna, query = my.atac,
                                    dims = 1:30, reference.reduction = "pca",features = hvgs)
  my.rna <- RunUMAP(my.rna, dims = 1:30, reduction = "pca", return.model = TRUE)
  my.atac <- MapQuery(anchorset = my.anchors, reference = my.rna, query = my.atac,
                      refdata = list(celltype = "annotation_fine"), 
                      reference.reduction = "pca", reduction.model = "umap")
  p1a <- DimPlot(my.rna, reduction = "umap", group.by = "annotation_fine", label = TRUE, label.size = 3,
                repel = TRUE) + NoLegend() + ggtitle("option1: scRNA (integrated)")
  p2a <- DimPlot(my.atac, reduction = "ref.umap", group.by = "annotation_fine", label = TRUE,
                label.size = 3, repel = TRUE) + NoLegend() + ggtitle("scATAC-seq imputed")
  print(p1a + p2a);gc()
  ...
  p3a <- DimPlot(my.rna, reduction = "xtsne", group.by = "annotation_fine", label = TRUE, label.size = 3,
                repel = TRUE) + NoLegend() + ggtitle("option2: scRNA (xtSNE)")
  p4a <- DimPlot(my.atac, reduction = "imputed.xtsne", group.by = "annotation_fine", label = TRUE,
                label.size = 3, repel = TRUE) + NoLegend() + ggtitle("scATAC-seq imputed")
  print(p3a + p4a)
  ...
  p3b <- DimPlot(my.rna, reduction = "umap", group.by = "annotation_fine", label = TRUE, label.size = 3,
                repel = TRUE) + NoLegend() + ggtitle("option2: scRNA (UMAP)")
  p4b <- DimPlot(my.atac, reduction = "imputed.umap", group.by = "annotation_fine", label = TRUE,
                label.size = 3, repel = TRUE) + NoLegend() + ggtitle("scATAC-seq imputed")
  print(p3b + p4b)
  
  my.embeds=list()
  my.embeds[["option1.mapquery"]]=my.atac@reductions$ref.umap@cell.embeddings
  ...
  #saveRDS(my.embeds,paste0("/scicore/home/tschoppp/GROUP/lab/skeletalconvergence/fromMenghan/scATAC/coembed/coembed_embeddings_",tmp.samples,".rds"))
...
my.embeds=readRDS(paste0("/scicore/home/tschoppp/GROUP/lab/skeletalconvergence/fromMenghan/scATAC/coembed/coembed_embeddings_",samples,".rds"))

DefaultAssay(subset.atac)="activity"
subset.atac[['mapquery.umap']] <- CreateDimReducObject(embeddings = my.embeds$option1.mapquery[rownames(subset.atac@reductions$lsi),], 
                                                  key = 'mapquery-UMAP_', assay = subset.atac@active.assay)
...
p1a=FeaturePlot(subset.atac,reduction = "mapquery.umap",features = c("pseudotime"),order = TRUE) + theme(aspect.ratio=1) +
  scale_colour_gradientn(colours = (brewer.pal(n = 9, name = 'YlGnBu'))) + ggtitle(paste0(samples,", scATAC mapQuery"))
# use same scale as scRNA data
