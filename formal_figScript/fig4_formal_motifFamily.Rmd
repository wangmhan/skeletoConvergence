...
mfunion.used$mfnew.unique=""
for(i in 1:nrow(mfunion.used)){
  tmp.id=mfunion.used$motifid[i]
  mfunion.used$mfnew.unique[i]=mf2tfnames.all$overall[mf2tfnames.all$overall$motifid==tmp.id,"mfnew.unique"]
}
```
...
```{r, message=FALSE, warning=FALSE, echo=TRUE,fig.width=5,fig.height=6}
stats=mf.lns
...
pdf(paste0("~/figure_drafts/MotifStats_freqBarplot.pdf"),width = 8,height = 6)
g1+ggtitle("cluster ID")+theme(text = element_text(size=15))
